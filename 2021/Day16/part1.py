...
def parsePacket(s, indent = ''):
  ...
  #print(indent + "id: " + str(id))
  if id == 4:
    literal = parseLiteral(s, start)
    print(indent + str(literal))
  elif take(1,s) == '0':
    size_len = int(take(15,s),2)
    asdf = s.i
    while s.i - asdf < size_len:
      # print( "going in again at " + str(s.i))
      version += parsePacket(s, " " * s.i)
    # print(indent + "len", s.i - asdf, s.i, asdf, size_len)
    #assert s.i - asdf == size_len
    # version += parseBinaryPacketStream(take(size_len,s))
    #IDC right now
  else:
    ...
    for i in range(sub_packets):
      version += parsePacket(s, " " * s.i)
    #IDC right now
  #print(indent + str("version"), version, s.b[start:s.i+1])
...
if __name__ == "__main__":
  ...
  assert parsePacketStream('8A004A801A8002F478') == 16
  assert parsePacketStream('620080001611562C8802118E34') == 12
  assert parsePacketStream('C0015000016115A2E0802F182340') == 23
  assert parsePacketStream('A0016C880162017C3686B18A3D4780') == 31
